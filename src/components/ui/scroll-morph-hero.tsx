...
export default function IntroAnimation({ onComplete }: { onComplete?: () => void }) {
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            ...
            let newScroll = scrollRef.current;
            if (e.deltaY > 0) {
    ...
    // Track scroll completion to exit
    useEffect(() => {
      const unsubscribe = virtualScroll.on("change", (latest) => {
        if (latest >= MAX_SCROLL - 10) {
          if (onComplete) {
            onComplete();
          }
        }
      ...
      return () => unsubscribe();
    }, [virtualScroll, onComplete]);

    return (
        <div ref={containerRef} className="relative w-full h-full bg-[#0A0A0A] overflow-hidden select-none">
            {/* Exit/Skip Button */}
            {onComplete && (
              <button
                onClick={onComplete}
                className="absolute top-6 right-6 px-4 py-1.5 rounded-full bg-[var(--surface-elevated)] border border-[var(--border)] text-xs font-bold uppercase tracking-widest text-[var(--text-secondary)] hover:text-white hover:border-[var(--primary)] transition-all cursor-pointer z-30 font-sans"
              >
                Skip Intro ➔
              </button>
            )}
            ...
            <div className="flex h-full w-full flex-col items-center justify-center perspective-1000">
                ...
                <div className="absolute z-0 flex flex-col items-center justify-center text-center pointer-events-none top-1/2 -translate-y-1/2">
                    <motion.h1
                        ...
                        className="text-3xl font-heading font-black tracking-tight text-white md:text-5xl uppercase"
                    >
                    ...
                    <motion.p
                        ...
                        className="mt-4 text-xs font-bold tracking-[0.2em] text-[var(--text-secondary)]"
                    >
                ...
                >
                    <h2 className="text-4xl md:text-6xl font-heading font-black text-white uppercase tracking-tight mb-4">
                        ENTER THE DOOM REALM
                    </h2>
                    <p className="text-xs md:text-sm text-[var(--text-secondary)] max-w-lg leading-relaxed font-sans">
                        Experience movies like never before. 4K Ultra HD Streaming, 1-Minute Micro-Dramas, and curated collections tailored for you. <br />
                        <span className="text-[var(--primary)] font-bold animate-pulse">Scroll to the end of the cards to enter doom.</span>
                    </p>
                </motion.div>
